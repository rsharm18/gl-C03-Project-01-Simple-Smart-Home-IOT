...
from model.ActiveConstants import ActiveTopics, ActiveMessageStatus
...
class Light_Device():
    ...
    def handle_topic(self, topic_name, payload):
        if topic_name == ActiveTopics.DEVICE_REGISTER_RESPONSE_TOPIC_NAME.value:
    ...
    def _handle_device_registration_response(self, payload: Device_Input):
        if payload.status == ActiveMessageStatus.SUCCESS.value:
            print("{} : registration successful!".format(self._device_id))
        else:
            print("{} : registration failed".format(self._device_id))
